...
public class SuppliersController : ControllerBase
{
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateSupplierRequest updated)
    {
        var s = await _db.Suppliers.FindAsync(id);
        ...
        // ── QARZNI TUZATISH (tahrirlash) ──────────────────────────
        // Boshlang'ich (ilovagacha bo'lgan) summa XATO kiritilgan bo'lsa, uni shu
        // yerda to'g'rilash mumkin. Faqat qiymat ANIQ yuborilganda qo'llaymiz —
        // aks holda (null) qarzga tegilmaydi (eski chaqiruvlar buzilmaydi).
        if (updated.DebtBalance.HasValue)
        {
            double corrected = Math.Round(updated.DebtBalance.Value, 2);
            if (corrected < 0) corrected = 0;
            s.DebtBalance = corrected;
            // Qarz to'liq yopilsa — boshlang'ich qarz eslatmasi ham keraksiz bo'ladi.
            if (s.DebtBalance <= 0.5) s.DebtReminderDone = true;
        }

        // Eslatma sanasini yangilaymiz.
...
// Firmani tahrirlash so'rovi. DebtBalance null bo'lsa — qarzga tegilmaydi;
// qiymat berilsa — qarz TUZATILADI (buxgalter xato kiritgan summani to'g'rilaganda).
