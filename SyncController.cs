...
public class SyncController : ControllerBase
{
    ...
    private async Task<SyncOpResult> ApplyClientPaymentAsync(SyncOpRequest op, double amount, string? device)
    {
        ...
        // CashierId = yig'uvchi admin (mobil) — shu pul uning bugungi kassasiga qo'shiladi.
        // Source = "Mobil" — ko'chada (offline) yig'ilib, sinxron orqali kelgan.
        _db.DebtPayments.Add(new DebtPayment
        {
            ...
            CashierId = (op.CollectorUserId.HasValue && op.CollectorUserId.Value > 0) ? op.CollectorUserId : null,
            PaymentType = "Cash",
            Source = "Mobil",
...
public class SyncOpRequest
{
    ...
    public int? CollectorUserId { get; set; }        // Pulni yig'gan xodim (admin) Id'si — kassaga qo'shish uchun
}
