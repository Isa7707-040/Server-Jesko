...
public class ClientsController : ControllerBase
{
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateClientRequest updated)
    {
        var c = await _db.Clients.FindAsync(id);
        ...
        // Qarzning o'zi odatda nasiya savdo / to'lov orqali boshqariladi. AMMO
        // boshlang'ich (ilovagacha bo'lgan) summa XATO kiritilgan bo'lsa, admin
        // uni shu yerda TO'G'RILAShi mumkin. Faqat qiymat ANIQ yuborilganda
        // qo'llaymiz — yuborilmasa (null), qarzga UMUMAN tegilmaydi (eski
        // xatti-harakat to'liq saqlanadi, orqaga mos).
        if (updated.DebtBalance.HasValue)
        {
            double corrected = Math.Round(updated.DebtBalance.Value, 2);
            if (corrected < 0) corrected = 0;
            c.DebtBalance = corrected;
            // Qarz 0 ga tushsa — boshlang'ich qarz eslatmasi keraksiz.
            if (c.DebtBalance <= 0.5) c.DebtReminderDone = true;
        }
    ...
    //  Kassir kassada qabul qilsa — CashierId va PaymentType ham saqlanadi,
    //  shunda bu pul kassirning "Mening sotuvlarim" va kunlik kassasiga qo'shiladi.
    // ───────────────────────────────────────────────────────────
    ...
    public async Task<IActionResult> PayDebt(int id, [FromBody] DebtPaymentRequest req)
    {
        ...
        var payment = new DebtPayment
        {
            ...
            // Kassada qabul qilingan bo'lsa — kassir ID va to'lov turini saqlaymiz.
            CashierId = (req?.CashierId.HasValue == true && req.CashierId.Value > 0) ? req.CashierId : null,
            PaymentType = NormalizePaymentType(req?.PaymentType),
            Source = string.IsNullOrWhiteSpace(req?.Source) ? "Kassa" : req!.Source!.Trim(),
    ...
    // ───────────────────────────────────────────────────────────
    //  QARZ YIG'IMLARI (KASSA SOLISHTIRUVI UCHUN)
    //  Berilgan sana oralig'idagi qarz to'lovlari. Ixtiyoriy cashierId bilan
    //  faqat bitta xodim (kassir yoki mobil yig'uvchi admin) qabul qilgan
    //  to'lovlarni qaytaradi. Kassir "Mening sotuvlarim" da bugungi qarz
    //  yig'imini ko'rsatib, kassa pulini solishtirishi uchun ishlatiladi.
    //  Admin esa kunlik umumiy kassani yopayotganda mobil yig'imlarni qo'shadi.
    // ───────────────────────────────────────────────────────────
    [HttpGet("collections")]
    public async Task<IActionResult> Collections([FromQuery] DateTime? from, [FromQuery] DateTime? to, [FromQuery] int? cashierId)
    {
        var q = _db.DebtPayments.AsQueryable();
        if (from.HasValue) q = q.Where(p => p.PaidAt >= from.Value);
        if (to.HasValue) q = q.Where(p => p.PaidAt <= to.Value.AddDays(1));
        if (cashierId.HasValue && cashierId.Value > 0) q = q.Where(p => p.CashierId == cashierId.Value);
        ...
        var list = await q
            ...
            .Select(p => new
            {
                p.Id,
                p.ClientId,
                ClientName = _db.Clients.Where(c => c.Id == p.ClientId).Select(c => c.Name).FirstOrDefault(),
                p.Amount,
                p.RemainingAfter,
                p.PaidAt,
                p.PaymentType,
    ...
    // To'lov turini normallashtirish: faqat "Cash" yoki "Card". Default — naqd.
    private static string NormalizePaymentType(string? type)
    {
        var t = (type ?? "").Trim();
        return t.Equals("Card", StringComparison.OrdinalIgnoreCase) ? "Card" : "Cash";
    }
}
...
public class DebtPaymentRequest
{
    ...
    public int? CashierId { get; set; }     // To'lovni qabul qilgan kassir (kassada)
    public string? PaymentType { get; set; } // Cash | Card (default Cash)
    public string? Source { get; set; }      // Kassa | Mobil (default Kassa)
}

// Mijozni tahrirlash so'rovi. DebtBalance null bo'lsa — qarzga TEGILMAYDI;
// aniq qiymat berilsa — qarz TUZATILADI (admin xato boshlang'ich summani
// to'g'rilaganda ishlatiladi).
